...
public class UserController {
	...
	public String login(@Valid @ModelAttribute("loginBean") LoginBean lb, Errors errors, HttpSession session) {
	  System.out.println("vpilogin.action" + lb.getUsername() +" " + Md5.GetMD5Code(lb.getPassword()));
		if(errors.hasFieldErrors())return "vpilogin";
		User user=null;
		try {
			user=userSvc.login(lb.getUsername());
			if(user==null || !user.getPassword().equals(Md5.GetMD5Code(lb.getPassword()))){
			//if(user==null || !user.getPassword().equals(lb.getPassword())){//(Md5.GetMD5Code(lb.getPassword())))
